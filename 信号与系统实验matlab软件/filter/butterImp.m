...
%   butterImpworth filter and returns the filter coefficients in length
%   N+1 vectors B and A.  The cut-off frequency Wn must be
%   0.0 < Wn < 1.0, with 1.0 corresponding to half the sample rate.
%
%   If Wn is a two-element vector, Wn = [W1 W2], butterImp returns an 
%   order 2N bandpass filter with passband  W1 < W < W2.
%   [B,A] = butterImp(N,Wn,'high') designs a highpass filter.
%   [B,A] = butterImp(N,Wn,'stop') is a bandstop filter if Wn = [W1 W2].
%   
%   When used with three left-hand arguments, as in
%   [Z,P,K] = butterImp(...), the zeros and poles are returned in
%   length N column vectors Z and P, and the gain in scalar K. 
%
%   When used with four left-hand arguments, as in
%   [A,B,C,D] = butterImp(...), state-space matrices are returned.
%
%   butterImp(N,Wn,'s'), butterImp(N,Wn,'high','s') and butterImp(N,Wn,'stop','s')
%   design analog butterImpworth filters.  In this case, Wn can be bigger
%   than 1.0.
%
...
%   $Revision: 1.18 $  $Date: 1997/02/06 21:51:56 $
...
% step 3: Get N-th order butterImpworth analog lowpass prototype
[z,p,k] = buttap(n);
